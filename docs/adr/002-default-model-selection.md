...
- 状态：已接受（Accepted）
- 日期：2026-08-09（2026-08-25 随 definition 机制移除而简化，见 ADR-006）
...
`Agent({ model })` 接受固定模型或有序候选列表，数组按顺序选择第一个当前可用候选。
...
显式 `Agent({ model })` 表达调用方确定意图，因此其候选全部不可用时创建失败。

### 4. Model is mutable after launch
