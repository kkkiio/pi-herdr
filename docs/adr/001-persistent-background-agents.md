...
Agent 使用正常落盘的全新 session，不 fork 创建者 transcript，也不提供临时 session 分支。
...
创建者会话重启会丢失创建来源内存。仍在 herdr 中运行的旧 Agent 继续可达，但在其他会话的 `ListAgents` 结果中归类为 peer。
...
常规 worktree 合并与清理由创建者在用户指示下使用原生 Git/Herdr 完成。pi-herdr 只在 Agent 创建失败回滚时以 `force: false` 先尝试移除本次新建的 worktree；安全移除失败后再关闭本次 pane。只有 runtime 已确认关闭，且 launch 已产生可验证的 `herdr:pi` session、绝对 `.jsonl` path 位于配置的 Pi session tree 内时，才删除这个精确文件。close 结果不确定或其他清理无法验证时保留现场并报告残留资源。
...
| Workspace-wide capacity               | 无共享 managed metadata 时无法准确区分不同会话创建的 Agent 与普通 peer;per-Primary 上限也已随实践移除 |
| 插件自动清理所有 worktree             | 合并状态与用户保留意图应由创建者、Git 和用户决定                                                 |
...
- 会话重启、peer 发现和 tab 关闭语义直接对齐 herdr live snapshot。
- Stop、消息和 worktree 清理边界明确，避免插件隐藏删除用户资源。

### Negative

- 已关闭 Agent 不能通过 `SendMessage` 自动恢复。
- reply address 与 `createdBy` 都是 live 信息，可能随 rename、pane 变化或创建者会话重启失效。
