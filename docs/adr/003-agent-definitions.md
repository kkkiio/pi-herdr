...
pi-herdr 需要发布 Explorer 和 General Purpose 两个开箱即用角色，并允许用户维护全局角色。更重要的是，Primary 应优先理解任务相关项目，发现当前或外部仓库提供的项目级角色，再显式选择最合适的角色定义和初始角色指令。角色配置必须可阅读、可验证，并映射到 pi 原生模型、工具、extensions 和 skills 能力。
...
`Agent` 使用 `definition: string`，不再使用只能表达裸名称的 `agent_type`。Selector 有两种无歧义形式：
...
pi-herdr 不根据 Primary 当前 Git root 自动选择项目角色，也不扫描外部仓库。`Agent` tool guideline 要求 Primary 先用普通文件与 Git 工具检查任务相关项目的 `.pi/agents`、`.agents/agents` 及 `AGENTS.md`，然后传入选中的明确路径；找不到合适项目角色时再使用 catalog。
